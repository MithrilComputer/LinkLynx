...
{
    internal static class SerialDispatcher
    {
        /// <summary>
        ...
        internal static bool AddToDispatcher(uint joinId, Action<PageLogicBase, SigEventArgs> action)
        {
            if (!dispatcher.ContainsKey(joinId))
            {
                ...
                CrestronConsole.PrintLine($"[SerialDispatcher] Join ID {joinId} bound to {action.Method.Name} in the dispatcher.");
                return true;
            }
            else
            {
                CrestronConsole.PrintLine($"[SerialDispatcher] Join ID {joinId} already exists in the dispatcher for {action.Method.Name}.");
                return false;
            }
        }
        ...
        internal static bool CheckIfDispatcherContainsKey(uint joinId)
        {
            if (dispatcher.ContainsKey(joinId))
            {
                CrestronConsole.PrintLine($"[SerialDispatcher] Join ID {joinId} exists in the dispatcher.");
                return true;
            }
            else
            {
                CrestronConsole.PrintLine($"[SerialDispatcher] Join ID {joinId} does not exist in the dispatcher.");
                return false;
            }
        }
        ...
        internal static Action<PageLogicBase, SigEventArgs> GetActionFromKey(uint joinId)
        {
            ...
            {
                CrestronConsole.PrintLine($"[SerialDispatcher] Found action {action.Method.Name} for join ID {joinId}.");
                return action;
            }
            else
            {
                CrestronConsole.PrintLine($"[SerialDispatcher] No action found for join ID {joinId}.");
                return null;
            }
        }
